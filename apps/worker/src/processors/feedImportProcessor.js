...
exports.processFeedImportJob = processFeedImportJob;
...
function feedProductSlug(storeId, externalId) {
...
async function tryFindExistingProductForItem(item) {
    const eanStr = item.ean != null && String(item.ean).trim() !== "" ? String(item.ean).trim() : null;
    const modelStr = item.modelNumber != null && String(item.modelNumber).trim() !== ""
        ? String(item.modelNumber).trim()
        : null;
    if (eanStr) {
        const byEan = await prisma_1.prisma.product.findFirst({
            where: { ean: eanStr }
        });
        if (byEan)
    ...
    if (modelStr) {
        const byModel = await prisma_1.prisma.product.findFirst({
            where: {
                modelNumber: { equals: modelStr, mode: "insensitive" }
            }
        });
        if (byModel)
            return { productId: byModel.id, source: "modelNumber" };
    }
    const titleWords = item.title.trim().split(/\s+/).slice(0, 3).join(" ");
    if (titleWords.length < 2)
        return null;
    const byTitle = await prisma_1.prisma.product.findMany({
        where: {
    ...
    for (const p of byTitle) {
        const sim = (0, string_similarity_1.compareTwoStrings)(normalizedItemTitle, normalizeTr(p.name));
...
async function matchProductForStoreProduct(storeProduct) {
    ...
    if (allCandidates.length === 0) {
        return { productId: null, score: 0, details: {} };
    }
    ...
    for (const product of allCandidates) {
        ...
        const titleSim = (0, string_similarity_1.compareTwoStrings)(normalizedStoreTitle, normalizedProductTitle);
        ...
        if (storeProduct.specsJson && product.specsJson) {
            const s = storeProduct.specsJson;
            const p = product.specsJson;
    ...
    return {
        ...
        score: bestScore,
        details: (bestDetails ?? { candidateProductId: 0, components: {}, total: 0 })
    };
}
/** Compare at 2 decimal places to avoid duplicate PriceHistory for same price. */
function priceChanged(latestPrice, newPrice) {
...
async function upsertOfferAndPriceHistory(storeProduct, productId, item, logContext) {
    const currentPriceNum = Number(item.price);
    const currentPrice = new client_1.Prisma.Decimal(Number.isFinite(currentPriceNum) ? currentPriceNum.toFixed(2) : "0");
    const originalPriceVal = item.originalPrice != null && Number.isFinite(Number(item.originalPrice))
        ? new client_1.Prisma.Decimal(Number(item.originalPrice).toFixed(2))
        : null;
    let logData = null;
    logData = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (offers.length > 0) {
            ...
            await tx.product.update({
                where: { id: productId },
                data: {
                    status: client_2.ProductStatus.ACTIVE,
    ...
    if (logContext && logData) {
        console.log(`[feed-import] updated_offer storeProductId=${logData.storeProductId} offerId=${logData.offerId} externalId=${logContext.externalId} productId=${productId} oldPrice=${logData.oldPrice} newPrice=${logData.newPrice} priceHistoryCreated=${logData.priceHistoryCreated} productCacheRecomputed=${logData.productCacheRecomputed}` +
            (logData.lowestPriceCache != null ? ` lowestPriceCache=${logData.lowestPriceCache} offerCountCache=${logData.offerCountCache}` : ""));
    }
}
async function processFeedImportJob(job) {
    const feedImport = await prisma_1.prisma.feedImport.findUnique({
    ...
    await prisma_1.prisma.feedImport.update({
        where: { id: feedImport.id },
        data: {
            status: client_2.FeedStatus.RUNNING,
            startedAt: new Date()
        }
    });
    try {
        const adapter = (0, feeds_1.getFeedAdapter)(feedImport.type, feedImport.store.slug);
        const content = await (0, feeds_1.readFeedFile)(feedImport.sourceRef ?? "");
        ...
        let errors = 0;
        const itemErrors = [];
        const logCtx = (externalId) => ({ externalId });
        for (let index = 0; index < items.length; index++) {
            const item = items[index];
            try {
                processed += 1;
                let storeProduct;
                let productIdForOffer = null;
                let reusedByProductId = false;
                const existingByExternalId = await prisma_1.prisma.storeProduct.findUnique({
                    where: {
                        storeId_externalId: {
                ...
                if (existingByExternalId) {
                    storeProduct = await prisma_1.prisma.storeProduct.update({
                        ...
                        data: {
                            ...
                            specsJson: (item.specs ?? existingByExternalId.specsJson),
                            url: item.url,
                    ...
                    const existingByStoreProduct = existingProduct &&
                        (await prisma_1.prisma.storeProduct.findFirst({
                            where: {
                                storeId: feedImport.storeId,
                    ...
                    if (existingByStoreProduct) {
                        storeProduct = await prisma_1.prisma.storeProduct.update({
                            ...
                            data: {
                                ...
                                specsJson: (item.specs ?? existingByStoreProduct.specsJson),
                                url: item.url,
                                imageUrl: item.imageUrl ?? existingByStoreProduct.imageUrl,
                                productId: existingProduct.productId,
                                matchStatus: client_2.MatchStatus.AUTO_MATCHED,
                        ...
                        updated += 1;
                        console.log(`[feed-import] reused_store_product externalId=${item.externalId} productId=${existingProduct.productId} by=${existingProduct.source}`);
                    }
                    else {
                        storeProduct = await prisma_1.prisma.storeProduct.create({
                            data: {
                                storeId: feedImport.storeId,
                                externalId: item.externalId,
                                title: item.title,
                                ...
                                matchStatus: client_2.MatchStatus.UNMATCHED,
                                matchScore: 0
                            }
                        });
                        created += 1;
                ...
                if (matchResult.productId && matchResult.score >= AUTO_MATCH_THRESHOLD) {
                    ...
                    await prisma_1.prisma.storeProduct.update({
                    ...
                    await upsertOfferAndPriceHistory(storeProduct, productIdForOffer, item, logCtx(item.externalId));
                    console.log(`[feed-import] matched_existing_product externalId=${item.externalId} productId=${matchResult.productId} score=${matchResult.score}`);
                }
                    ...
                    const existingPendingReview = await prisma_1.prisma.unmatchedProductReview.findFirst({
                        where: {
                            storeProductId: storeProduct.id,
                            status: client_2.UnmatchedStatus.PENDING
                        }
                    });
                    if (!existingPendingReview) {
                        await prisma_1.prisma.unmatchedProductReview.create({
                            data: {
                                storeProductId: storeProduct.id,
                        ...
                        await prisma_1.prisma.unmatchedProductReview.update({
                    ...
                    productIdForOffer = matchResult.productId;
                    await upsertOfferAndPriceHistory(storeProduct, productIdForOffer, item, logCtx(item.externalId));
                    console.log(`[feed-import] matched_existing_product externalId=${item.externalId} productId=${matchResult.productId} score=${matchResult.score} (review)`);
                }
                    ...
                    if (existingProduct) {
                        ...
                        await upsertOfferAndPriceHistory(storeProduct, productIdForOffer, item, logCtx(item.externalId));
                        console.log(`[feed-import] matched_existing_product externalId=${item.externalId} productId=${existingProduct.productId} by=${existingProduct.source}`);
                    }
                    else {
                        const slug = feedProductSlug(feedImport.storeId, item.externalId);
                        let newProduct = await prisma_1.prisma.product.findUnique({ where: { slug } });
                        if (!newProduct) {
                            newProduct = await prisma_1.prisma.product.create({
                                data: {
                                    name: item.title,
                                    slug,
                                    ean: item.ean ?? null,
                        ...
                        await upsertOfferAndPriceHistory(storeProduct, productIdForOffer, item, logCtx(item.externalId));
                        console.log(`[feed-import] created_new_product externalId=${item.externalId} productId=${newProduct.id} slug=${slug}`);
                    }
                }
                else {
                    await prisma_1.prisma.storeProduct.update({
        ...
        await prisma_1.prisma.feedImport.update({
            where: { id: feedImport.id },
            data: {
                status: errors > 0 ? client_2.FeedStatus.PARTIAL : client_2.FeedStatus.SUCCESS,
                finishedAt: new Date(),
        ...
        await prisma_1.prisma.feedImport.update({
            where: { id: feedImport.id },
            data: {
                status: client_2.FeedStatus.FAILED,
                finishedAt: new Date(),
...
//# sourceMappingURL=feedImportProcessor.js.map