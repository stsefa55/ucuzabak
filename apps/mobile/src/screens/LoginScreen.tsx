...
export function LoginScreen() {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const tokens = await login({ email: e, password });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            ...
            >
              <Text style={styles.primaryBtnTxt}>{loading ? "Giriş yapılıyor..." : "Giriş Yap"}</Text>
            </Pressable>

            <View style={styles.secondaryRow}>
              <Text style={styles.secondaryTxt}>Hesabınız yok mu?</Text>
              <Pressable
                onPress={() => navigation.navigate("Register")}
                accessibilityLabel="Kayıt ol"
              >
                <Text style={styles.linkTxt}>Kayıt Ol</Text>
              </Pressable>
            </View>
          </View>
        </ScrollView>
